...
class TestAuth(unittest.TestCase):
    ...
    def test_login(self):
        ...
        registration_response = self.app.post('/register', data=json.dumps(user_data), content_type='application/json')
        self.assertEqual(registration_response.status_code, 201)
        response = self.app.post('/login', data=json.dumps(user_data), content_type='application/json')
        data = json.loads(response.data.decode('utf-8'))
        self.assertEqual(response.status_code, 200)
